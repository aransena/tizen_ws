...
static bool send_UDP(const char* message, void *data){
	...
	if (sendto(ad->udpPort,message,sizeof(message),0,
...
Eina_Bool _rotary_handler_cb(void *data, Eext_Rotary_Event_Info *ev)
{
	...
	if (ev->direction == EEXT_ROTARY_DIRECTION_CLOCKWISE)
	{
	  ...
	  if(send_UDP("CW",ad)){
		  dlog_print(DLOG_DEBUG, LOG_TAG, "ROTARY HANDLER: Clockwise UDP message sent");
	  }
	  else{
		  app_terminate(ad);
	  }
	...
	else
	{
	  ...
	  if(send_UDP("CCW",ad)){
		  dlog_print(DLOG_DEBUG, LOG_TAG, "ROTARY HANDLER: Counter clockwise UDP message sent");
	  }
	  else{
		  app_terminate(ad);
	  }
...
static void get_UDPsocket(const char* hostname, const char* portname,appdata_s *ad){
